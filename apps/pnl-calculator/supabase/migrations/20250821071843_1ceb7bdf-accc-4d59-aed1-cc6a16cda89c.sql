-- Enhanced function to track only user-modified fields (exclude auto-calculated ones)
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    ELSE
      ...
      IF jsonb_object_keys(field_changes) IS NOT NULL AND array_length(array(SELECT jsonb_object_keys(field_changes)), 1) > 0 THEN
        INSERT INTO public.version_history (version_id, product_id, action_type, change_description, changed_fields, user_email, user_id)
        VALUES (
          NEW.id,
          NEW.product_id,
          ...
          array_length(array(SELECT jsonb_object_keys(field_changes)), 1)::text || ' field changes' ||
